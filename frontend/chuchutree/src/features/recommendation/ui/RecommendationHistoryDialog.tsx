...
import { useBanProblem, useUnbanProblem, useGetBannedProblems, useGetRecommendHistory } from '@/entities/recommendation';
...
export function RecommendationHistoryDialog({ onClose }: props) {
  const { showFilters, toggleFilter, resetFilters } = useRecommendationStore();
  const { selectedDate } = useCalendarStore();

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useGetRecommendHistory();
  ...
  const { mutate: banProblem } = useBanProblem();
  const { mutate: unbanProblem } = useUnbanProblem();
  ...
  const handleToggleProblem = (problemId: number, problemTags: any[]) => {
    ...
    const problemIds = isRegistered
      ? willSolveProblems.filter((p) => p.problemId !== problemId).map((p) => p.problemId)
      : [...willSolveProblems.map((p) => p.problemId), problemId];
    const successMessage = isRegistered ? `문제 ${problemId}를 제거했습니다.` : `문제 ${problemId}를 추가했습니다.`;

    updateWillSolve.mutate(
      { date: formatDateString(selectedDate), problemIds },
      {
        onSuccess: () => {
          toast.success(successMessage);
          if (!isRegistered && problemTags.length > 0) {
  ...
  const handleToggleBanProblem = (problemId: number, problemTitle: string) => {
    ...
    if (isBanned) {
      unbanProblem(
        { problemId },
        {
          onSuccess: () => toast.success(`${problemTitle} 문제가 추천에 포함됩니다.`),
          onError: () => toast.error('문제 제외 취소에 실패했습니다.'),
        },
      );
    } else {
      banProblem(
        { problemId },
        {
          onSuccess: () => toast.success(`${problemTitle} 문제가 추천에서 제외됩니다.`),
          onError: () => toast.error('문제 제외에 실패했습니다.'),
        },
      );
    }
  };
                              ...
                              <div className="flex min-w-6 flex-col items-end">
                                <AppTooltip content={isBanned ? '문제 추천에 포함' : '문제 추천에서 제외'} side="left">
                                  <div
                                    ...
                                    role="button"
                                    tabIndex={0}
                                    onClick={(e) => { e.stopPropagation(); handleToggleBanProblem(problem.problemId, problem.problemTitle); }}
                                    onKeyDown={(e) => { if (e.key === 'Enter') { e.stopPropagation(); handleToggleBanProblem(problem.problemId, problem.problemTitle); } }}
                                  >
                                    {isBanned ? (
                                ...
                                {showFilters.recommendReason && problem.recommandReasons.length > 0 && (
                                  <p className="text-muted-foreground line-clamp-1">
                                    {showFilters.algorithm
                                      ...
                                      : problem.recommandReasons[0].reason.replace(/'[^']+'/g, '').replace(/\s+/g, ' ').trim()}
                                  </p>
                                )}
                              </div>
                            )}
