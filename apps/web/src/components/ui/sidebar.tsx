...
function SidebarProvider({
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      // event.code 사용 (물리적 키 위치, 키보드 레이아웃/IME 영향 없음)
      if (event.code === 'KeyB' && event.shiftKey && !event.ctrlKey && !event.metaKey && !event.altKey) {
...
function Sidebar({
  ...
  // 모바일에서도 항상 icon 형태로 표시 (Sheet 사용 안 함)
  // if (isMobile) {
  //   return (
  ...
  return (
    <main
      ...
      className={cn(
        ...
        'peer-data-[variant=inset]:m-2 peer-data-[variant=inset]:ml-0 peer-data-[variant=inset]:rounded-xl peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2',
        className,
      )}
      {...props}
    />
